...
def main():
    ...
    scores.to_csv("Feature_Select_Scores")
    #print(scores)
    ...
    logit_acc.to_csv('logit_accuracy')
    logit_std.to_csv('logit_standard_dev')
    percep_acc, percep_std = get_feat_acc(X_train, y_train, indices, clf2)
    percep_acc.to_csv('percept_accuracy')
    percep_std.to_csv('percept_standard_dev')
    NB_acc, NB_std = get_feat_acc(X_train, y_train, indices, clf3)
    NB_acc.to_csv('NB_accuracy')
    NB_std.to_csv('NB_standard_dev')
    SVC_acc, SVC_std = get_feat_acc(X_train, y_train, indices, clf4)
    SVC_acc.to_csv('SVC_accuracy')
    SVC_std.to_csv('SVC_standard_dev')
    Rand_For_acc, Rand_For_std = get_feat_acc(X_train, y_train, indices, clf5)
    Rand_For_acc.to_csv('Rand_For_accuracy')
    Rand_For_std.to_csv('Rand_For_standard_dev')
    KNN_acc, KNN_std = get_feat_acc(X_train, y_train, indices, clf6)
    KNN_acc.to_csv('KNN_accuracy')
    KNN_std.to_csv('KNN_standard_dev')
